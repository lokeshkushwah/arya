...
if ($current >= $endtime) {
	echo '<h2>Exam date is over by : ' .$end.'<h2>';
	 ?>
	 <a href="<?php echo \yii\helpers\Url::home() ?>" class="btn btn-default btn-success">Go Back</a>
	 <a href="<?php echo \yii\helpers\Url::to(['examination/result', 'exam' => $exam]); ?>" class="btn btn-default btn-success">View Result</a>
	 <?php
}

if ($current >= $starttime && $current <= $endtime && $user_reg_for_exam > 0) { 
?>
</div><div class="container" style="background:#D1F8CE;margin-top:-140px">
<div class="col-md-12" style="background:#FE9221 " >
    <div class="col-md-8 " style="background: #FE9221;color: white;font-size: 24px;padding: 15px;font-weight: 800">Exam :<b> <?php echo ucwords($this->title=$exam); ?></b></div>
<div class="col-md-4"><div class="countdown simple" <?php //echo $end;           ?> data-date="<?php echo $new_date; //'June 7, 2037 15:03:25';            ?> " style="background: #FE9221;color: white;font-size: 18px;padding: 18px;font-weight: 800"></div></div>
</div>
            ...
            <div id="<?php echo 'tab'.str_replace(' ' ,'-',ucwords($subject->name)); ?>">
                            ...
                            <div class="content">
                                ...
                                <ul style="height:267px;overflow-y:  scroll; background: whitesmoke;padding-left: 15px ">
                                    ...
                                    for ($x = 1; $x <= $totalquestion; $x++) {
                                        ?>
                                    <li><a href="#<?php echo 'q'.str_replace(' ' ,'-',   ucwords($subject->name)) . $x; ?>" data-toggle="tab"><?php
                                                if ($x < 10)
                                                    echo "0";
                                                echo $x;
                                                ?> </a></li>
                ...
                <div class="col-md-6 pull-left"> 
                    <div class="tab-content">
                        ...
                        // echo $subject->name . '<br/>';
                        $n = 1;
                        $exam_paper_rel = $connection->createCommand("select * from `exan_paper_relation` where `exam_id`= '$exam_id'  and `subject_id`=' $subject_id' ")->queryAll();
                        foreach ($exam_paper_rel as $ex) {
                            ...
                            <div class="tab-pane" id="<?php echo 'q'.str_replace(' ' ,'-',  ucwords($subject->name)) . $n; ?>">
                                ...
                                <button class="canceled btn-danger" qn="<?php echo $n; ?>" qt="<?php echo $question->id; ?>">cancel </button><br/>
                                </div>
...
<div class="container center-block text-center">
    <a href="<?php echo \yii\helpers\Url::to(['examination/result', 'exam' => $exam]); ?>" class="btn btn-default btn-success">Submit</a>
